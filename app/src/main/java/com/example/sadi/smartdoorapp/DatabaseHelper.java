...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE PASSWORDS ( Password_ID INTEGER PRIMARY KEY AUTOINCREMENT, Username TEXT NOT NULL, Email_Address TEXT NOT NULL, PW TEXT NOT NULL, UNIQUE(Username, Email_Address) );");
        ...
        db.execSQL("CREATE TABLE USER ( User_ID INTEGER PRIMARY KEY AUTOINCREMENT, FName TEXT NOT NULL, LName TEXT NOT NULL, DOB TEXT, Image NONE, City TEXT, Privilege INTEGER NOT NULL DEFAULT 1, Num_of_Doors INTEGER NOT NULL DEFAULT 1, Password_ID INTEGER NOT NULL, Sec_Ques_ID INTEGER NOT NULL, FOREIGN KEY(Password_ID) REFERENCES PASSWORDS(Password_ID), FOREIGN KEY(Sec_Ques_ID) REFERENCES SECURITY_QUESTION(Sec_Ques_ID), UNIQUE(Password_ID, Sec_Ques_ID), CHECK(Privilege IN (1,2)) );");

        db.execSQL("CREATE TABLE PHONE ( Phone_ID INTEGER PRIMARY KEY AUTOINCREMENT, MAC_Address TEXT NOT NULL UNIQUE, IP_Address_Network TEXT NOT NULL, IP_Address_Device TEXT NOT NULL, Phone_Num TEXT NOT NULL, User_ID INTEGER NOT NULL, FOREIGN KEY(User_ID) REFERENCES USER(User_ID) );");
        ...
        db.execSQL("CREATE TABLE DEVICE ( Device_ID INTEGER PRIMARY KEY AUTOINCREMENT, MAC_Address TEXT NOT NULL, IP_Address_Network TEXT NOT NULL, IP_Address_Device TEXT NOT NULL, Door_Type TEXT NOT NULL, Device_Status TEXT NOT NULL DEFAULT 'Enable', Door_Status TEXT NOT NULL DEFAULT 'Close', Phone_ID INTEGER NOT NULL, Device_Name_ID INTEGER NOT NULL, FOREIGN KEY(Phone_ID) REFERENCES PHONE(Phone_ID), FOREIGN KEY(Device_Name_ID) REFERENCES DEVICE_NAME(Device_Name_ID), UNIQUE (MAC_Address, Device_Name_ID), CHECK(Door_Type IN ('Home', 'Car')), CHECK(Door_Status IN('Close', 'Open')), CHECK(Device_Status IN('Enable', 'Disable')) );");
